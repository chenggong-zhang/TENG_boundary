...
def build_cov_matrix(L_para, L_diag, dim):
    L = jnp.zeros((dim, dim), dtype=float)
...
class INNwProb(nn.Module):
    ...
    def __call__(self, x, evaluate=True, inv=False):
        if evaluate:  # used for evaluation
            ...
            p_latent_log = self.latent_space_dist(z - self.offset, {"S": build_cov_matrix(self.L, self.L_diag, self.dim), "mu": self.mu, "dist_params": self.dist_params})
            return p_latent_log + log_jac
        else:  # used for sampling
            p_latent_log = self.latent_space_dist(x - self.offset, {"S": build_cov_matrix(self.L, self.L_diag, self.dim), "mu": self.mu, "dist_params": self.dist_params})
...
if __name__ == "__main__":
    ...
    z = sINN.apply(param, s, inv=False)[0]
    x = sINN.apply(param, z, inv=True)[0]
    print(z)
    print(x)