...
class AbstractVarState(ABC):
    ...
    # def log_psi(self, samples, pmap=True):
    #     """
    #     evaluates the log_psi at the provided samples. Can be unnormalized
    #     pmap: whether to use the pmapped version of the function
    #     """
    #     if pmap:
    #         return self.log_psi_pmapped(samples)
    #     else:
    #         return self.log_psi_nonpmapped

    pure_funcs = None

    def __call__(self, samples):
        # warnings.warn('Calling var_state directly calls the log_psi function, it is preferable to call the log_psi function directly for clarity')
        return self.evaluate(samples)

    # @abstractmethod
    # def log_psi(self, samples) -> Array:
    #     """
    #     should be pmapped
    #     """
    #     pass
    #
    # @abstractmethod
    # def jac_log_psi(self, samples) -> Array:
    #     pass
    #
    # @abstractmethod
    # def jvp_vjp_log_psi_func(self, samples, return_value=False) -> Tuple[Callable, Callable, Array]:
    #     """
    #     returns: jvp
    #              vjp
    #              value
