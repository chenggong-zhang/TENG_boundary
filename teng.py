...
def get_config():
    ...
    ### model configs ###
    parser.add_argument("--load_model_state_from", type=str, nargs='?', default='model_state_circ_2d.pickle')
...
class CompareWithExact:
    def __init__(self, points_per_dim=512, config=None):
        ...
        self.xs = grid2d_inside_disk
        # print(f"This is the grid_2d_inside_disk")
...
def boundary_target_func(boundary_samples):
    ...
    return jnp.zeros((boundary_samples.shape[0], boundary_samples.shape[1]))
...
def euler_step(config, fiters, T, step, dt, var_state_new, var_state_old, var_state_temps, pde_operator, policy_grad, policy_grad2):
    ...
    samples, (boundary_samples, boundary_sqrt_weights), sqrt_weights = var_state_old.sampler.sample(angles=0)
    ...
    for iter in range(config.nb_iters_per_step + 2):
        ...
        u_target = jnp.concatenate([u_target, boundary_target], axis=1)
...
def heun_step(config, fiters, T, step, dt, var_state_new, var_state_old, var_state_temps, pde_operator, policy_grad, policy_grad2):
    ...
    var_state_old.set_state(var_state_new.get_state())
    samples, _, sqrt_weights = var_state_old.sampler.sample(angles=0)
    final_losses = []
...
def main():
    ...
    sampler = CircularQuadratureSampler(nb_sites=2, nb_samples=config.nb_samples, radius=1, quad_rule=None, rand_seed=config.sampler_seed)
